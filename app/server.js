...
app.get('/', (req, res) => {
    res.sendFile(__dirname + '/views/pages/login_page.html');
});
...
app.get('/create_user.html', (req, res) => {
    // Serve the HTML file
    res.sendFile(__dirname + '/views/pages/create_user.html');
});
            ...
            chat.getAllConversationsForUser(loggedInUserID).then(conversations => {
                ...
                chat.getAllOtherUsers(loggedInUserID).then(otherUsers => {
                    ...
                    chat.getAllMessagesInConversation(conversationID).then(messages => {
                        ejs.renderFile(__dirname + '/views/pages/user_page.ejs',
                            {
...
app.get('/conversation/:conversationId', (req, res) => {
    ...
    try {
        chat.getAllMessagesInConversation(conversationID).then(messages => {
            ...
            ejs.renderFile(__dirname + '/views/pages/conversation_page.ejs', { messages: messages, partner: { username: 'Ivan', icon: 'xx' }, conversationID: conversationID }, (err, html) => {
                if (err) {
                    console.error('Error rendering template:', err);
                    res.status(500).send('Internal Server Error');
                } else {
